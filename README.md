...
**访问控制**：需使用用户名密码登录（JWT）。账号由 **admin** 在「用户管理」中创建，无自助注册。角色分为 `admin`（集群与用户管理）、`operator`（扩缩容与批量操作）、`viewer`（只读）。
...
### 操作审计 (Audit Log)
- **操作记录**：记录所有通过 Dashboard 执行的写操作（扩缩容、删除、更新等），支持按操作类型筛选和分页查看；`operator` 字段为实际操作者用户名

### 用户与权限
- **登录**：用户名 + 密码，JWT 会话（默认 8 小时）
- **用户管理**（仅 admin）：创建/禁用/删除用户、重置密码
- **首启**：数据库无用户时，根据 `INIT_ADMIN_USERNAME` / `INIT_ADMIN_PASSWORD` 自动创建 admin
...
1. 打开 http://localhost:5173，在登录页使用 admin 账号登录
2. （admin）点击「添加集群」粘贴 kubeconfig 内容
3. 在侧边栏选择集群，进入对应资源页面（默认显示集群概览 Dashboard）
4. （operator/admin）在「定时扩缩容」页面配置 Cron 任务
5. （admin）在「用户管理」为其他运维创建 `operator` / `viewer` 账号
...
openssl rand -hex 32   # 用作 JWT_SECRET_KEY

cp .env.example .env
# 编辑 .env：填写 JWT_SECRET_KEY、KUBECONFIG_ENCRYPTION_KEY、INIT_ADMIN_PASSWORD 等

docker compose up -d --build
```

访问 http://localhost:5173 ，使用 `.env` 中的 init admin 登录。SQLite 数据持久化在 Docker 卷 `backend-data`。

> **注意**：backend 须 **单副本** 运行（定时任务使用进程内 APScheduler），请勿 `uvicorn --workers 2` 或多 Pod 无锁部署。
...
| `JWT_SECRET_KEY` | JWT 签名密钥（生产必填） | 开发占位值 |
| `JWT_EXPIRE_MINUTES` | JWT 有效期（分钟） | `480` |
| `INIT_ADMIN_USERNAME` | 首启 bootstrap 管理员用户名 | `admin` |
| `INIT_ADMIN_PASSWORD` | 首启 bootstrap 管理员密码 | 开发默认 `admin123` |
| `KUBECONFIG_ENCRYPTION_KEY` | kubeconfig 加密密钥（Fernet） | `DEBUG=true` 时由 `SECRET_KEY` 派生 |
| `SECRET_KEY` | 仅开发环境 kubeconfig 派生输入 | `dev-secret-change-in-production` |
| `CORS_ORIGINS` | 允许的前端来源（逗号分隔） | `http://localhost:5173,...` |
| `DEBUG` | 调试模式（日志/SQL echo） | `True` |

生产环境务必设置 `KUBECONFIG_ENCRYPTION_KEY` 与 `JWT_SECRET_KEY`：
...
|------|------|------|------|
| id | INTEGER | PRIMARY KEY, AUTOINCREMENT | 主键 |
| operator | VARCHAR(100) | NOT NULL | 操作者用户名 |
...
启动后端后访问 http://localhost:8082/docs 查看 Swagger API 文档。业务接口需在 **Authorize** 中填入 `Bearer <JWT>`（登录接口返回的 `access_token`）。
...
| `WebSocket /api/logs/ws/{cluster_id}/pod?access_token=<JWT>&namespace=...&pod_name=...` | 流式 Pod 日志（需 JWT） |
...
- 除 `GET /`、`POST /api/auth/login` 与文档外，所有 `/api/*` 需有效 JWT，否则返回 401
- 按角色 RBAC：`viewer` 只读；`operator` 可扩缩容与批量操作；`admin` 可管理集群与用户
- kubeconfig 使用 Fernet 对称加密存储，密钥不落库；`DEBUG=false` 时必须设置 `KUBECONFIG_ENCRYPTION_KEY`
- 生产环境必须配置 `JWT_SECRET_KEY`，并修改 `INIT_ADMIN_PASSWORD`；首登后创建个人账号
- API 响应不包含 kubeconfig 明文；WebSocket 日志需 query 参数 `access_token`
