...
- **Pod 事件查询**：查看特定 Pod 的所有 Kubernetes 事件，Warning 事件高亮显示
...
### 交互体验升级 (UX for Ops)
- **批量操作**：支持批量重启、删除 Pod，批量更新 Labels
...
│   │   ├── views/                # 页面组件
│   │   │   ├── Dashboard.vue      # 集群概览
│   │   │   ├── NodeMetrics.vue    # 节点概览
│   │   │   ├── ClusterDetail.vue  # 集群详情/资源列表
│   │   │   ├── ClusterList.vue    # 集群列表
│   │   │   ├── ScheduleList.vue   # 定时任务列表
│   │   │   └── AuditLog.vue       # 审计日志页面
│   │   ├── components/           # 组件
│   │   │   ├── LogViewer.vue      # 日志查看组件
│   │   │   └── YamlDiff.vue       # YAML Diff 组件
│   │   ├── api/                  # 接口封装
│   │   ├── stores/               # Pinia 状态
│   │   ├── router/               # 路由配置
│   │   └── styles/               # 全局样式
