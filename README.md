...
- **集群概览 Dashboard**：展示集群基本信息（节点数、命名空间数）、Pod 运行状态统计（总数/运行中/等待/异常）、Deployment/StatefulSet 数量、节点 CPU/内存使用量（需集群安装 metrics-server）、Pod CPU/内存请求量/Limit 量/实际使用量汇总及明细（需 metrics-server 支持）
...
3. 在侧边栏选择集群，进入对应资源页面（默认显示概览 Dashboard）
...
| 接口 | 说明 |
|------|------|
| `GET /api/resources/{cluster_id}/metrics` | 集群 metrics 汇总（兼容旧接口） |
| `GET /api/resources/{cluster_id}/metrics/overview` | 集群基础统计（节点数、命名空间数、Pod 统计、Deployment/StatefulSet 数量） |
